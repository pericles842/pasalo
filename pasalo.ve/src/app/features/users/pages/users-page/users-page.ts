...
export class UsersPage implements OnInit {
  ...
  success_message = signal<string | null>(null);
  ...
  loadUsers(): void {
    ...
    this.usersService.getCompanyUsers().subscribe({
      ...
      error: (err) => {
        this.error_message.set(err?.error?.error ?? 'No pudimos cargar los usuarios.');
        this.is_loading.set(false);
      }
    });
  }
  ...
  createUser(): void {
    ...
    this.success_message.set(null);

    if (this.user_form.invalid) {
      this.error_message.set('Revisa los datos del usuario antes de guardarlo.');
      return;
    }
    ...
    this.usersService.createUser(this.user_form.getRawValue()).subscribe({
      next: (response) => {
        ...
        this.success_message.set(`Usuario ${response.user.first_name} creado como ${response.role.name}.`);
        this.user_form.reset();
        this.loadUsers();
      },
      error: (err) => {
        this.is_saving.set(false);
        this.error_message.set(err?.error?.error ?? 'No pudimos crear el usuario.');
  ...
  changePlan(plan: PlanInterface): void {
    ...
    this.usersService.changePlan(plan.id).subscribe({
      next: (response) => {
        ...
        this.success_message.set(`Ahora tienes el ${response.plan.name}: hasta ${response.plan.user_limit} usuarios.`);
      },
      error: (err) => {
        this.is_changing_plan.set(false);
        this.error_message.set(err?.error?.error ?? 'No pudimos cambiar el plan.');
      }
    });
  }
}
